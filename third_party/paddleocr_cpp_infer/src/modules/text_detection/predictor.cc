...
absl::Status TextDetPredictor::Build() {
  ...
  resize_param.max_side_limit = params_.max_side_limit;
  // PP-OCRv6 inference.yml may set DetResizeForTest: null (no resize_long).
  // Prefer CLI/pipeline limit_side_len; only read resize_long when present.
  {
    auto it = pre_tfs.find("DetResizeForTest.resize_long");
    if (it != pre_tfs.end() && it->second != "null" && !it->second.empty()) {
